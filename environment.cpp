...
Environment::Environment() {
    ...
    this->define("and", makeBinaryBoolFn(std::logical_and<bool>()));
    this->define("or", makeBinaryBoolFn(std::logical_or<bool>()));
    this->define("<", makeCmpFn(std::less<float>()));
    this->define("<=", makeCmpFn(std::less_equal<float>()));
    this->define(">", makeCmpFn(std::greater<float>()));
    this->define(">=", makeCmpFn(std::greater_equal<float>()));
    this->define("=", makeCmpFn(std::equal_to<float>()));
    this->define("+", makePlusMulFn(std::plus<float>()));
    this->define("*", makePlusMulFn(std::multiplies<float>()));
    this->define("-", SubFn);
    this->define("/", DivFn);
